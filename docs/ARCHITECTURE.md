...
| 6 | Self-extending `FLAME_API.md` via `learn_pattern` | Self-improvement | Successful executions whose top RAG score was below threshold become indexed knowledge for future sessions, closing the loop on alucinations. Model-gated: only Sonnet/Opus (`WRITE_ALLOWED_MODELS`) write directly; local-model attempts stage to `candidates.json` for human review. | `src/flame_mcp/server.py::learn_pattern` (~line 1484) | pre-Chat-28 |
