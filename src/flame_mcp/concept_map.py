...
CONCEPT_MAP: list[dict[str, str]] = [
    ...
    },
    {
        ...
        "recipe": (
            ...
            "   a) ensure the Write File is media-only, versioned and NAMED "
            "'<Shot>_<step>': run the execute_plan op fix_comp_writefile "
            "(args: clip_path, step = the comp Step's short_name from SG, "
            "e.g. 'CMP'; idempotent, safe to repeat) and relay its name/set/"
            "skipped report — the rendered folder and frames must read "
            "<Shot>_<step>_v<version> (in-vivo Chat 99 a literal 'writefile' "
            "leaked into the PublishedFile code). FRAME ALIGNMENT is "
            "AUTOMATIC: the op derives start_frame from the conformed clip's "
            "source feed (never from an assumption), applies it to the "
            "batch, reads the batch start + Write File range BACK and prints "
            "'ALIGNMENT: … -> OK' or '… -> MISALIGNED — do not render'. GATE: "
            "render ONLY on OK; on MISALIGNED stop and show the operator the "
            "line (in-vivo Chat 99: an omitted start_frame left the batch at "
            "1, the comp rendered 0001-0100 against a 1001-1100 source and "
            "the COMP flip showed 'no media'). Relay any 'OVERWRITE WARNING' "
            "line — it means Follow Iteration would overwrite a rendered "
            "version: iterate first (b). setup_comp_batch derives the same "
            "start_frame at creation;\n"
            "   b) render: the render_batch tool (Background Reactor), the "
            "documented-safe route. RE-RENDERS: version_mode is 'Follow "
            "Iteration', so ITERATE the batch first (flame.batch.iterate(), "
            "idle event) or the render OVERWRITES the same version — never "
            ...
            "never publish a partial sequence. NUMBERING CHECK: the first "
            "rendered filename's frame number MUST equal the source's first "
            "frame (e.g. .1001.exr, not .0001.exr); if it does not, STOP — "
            "do not publish, do not regenerate the clip — and report the "
            "mismatch (a misnumbered comp version breaks the flip);\n"
            ...
            "   f) REVIEW VERSION — the delivery is not finished until it is "
            "reviewable in Flow Production Tracking (operator decision, Chat "
            "99). Mirror what the pipeline already does for Light (read one "
            "of its Versions if in doubt) — NEVER invent paths or field "
            "names:\n"
            "      1. resolve the movie path with tk_resolve_path, template "
            "'movie_shot_publish' on the Shot -> "
            "<shot_root>/review/<Shot>_<name>_v<version>.mov (the Step's "
            "short name, e.g. CMP). Do not compose it by hand;\n"
            "      2. import the RENDERED VERSION FOLDER into a desktop reel "
            "and export it with the canonical FPT review preset: "
            "<presets_root>/export/presets/shotgun/movie_file/"
            "'Submit for review.xml' (type=movie, QuickTime H.264 — the very "
            "preset the native ShotGrid integration uses). Resolve the "
            "presets root from get_flame_version (…/presets/<version>/…), "
            "never hardcode a version number. Import the version FOLDER, not "
            "the multi-version .clip: no version flipping to get wrong;\n"
            "      3. the export is ASYNCHRONOUS — after 'Export scheduled', "
            "poll until the .mov exists and its size stops growing. Never "
            "register or upload media that is still being written;\n"
            "      4. sg_create a Version: code=<Shot>_<STEP>_v<version>, "
            "entity=the Shot, sg_task=the compositing Task, "
            "sg_path_to_frames=the %04d EXR path (the same one the "
            "PublishedFile carries), sg_path_to_movie=the exported mov, "
            "sg_first_frame/sg_last_frame=the source's first/last frame "
            "(1001/1100 — the same anchor everything else uses), "
            "sg_status_list='rev', published_files=[the EXR PublishedFile];\n"
            "      5. sg_upload the mov into the Version's "
            "'sg_uploaded_movie' field. THIS is the streaming media: without "
            "it the Version has no thumbnail and cannot be played or "
            "reviewed in the web UI, so the delivery is incomplete. Report "
            "the Version id;\n"
            "   g) report DONE and tell the operator the one manual step "
