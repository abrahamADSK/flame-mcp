...
CONCEPT_MAP: list[dict[str, str]] = [
    ...
    },
    {
        ...
        "recipe": (
            ...
            "   d) KNOWN GAPS, fill them explicitly: the Version and both "
            "publishes carry NO Task (the batch template has no Step token, "
            "so the context resolves a Shot only) — link the comp Task with "
            "sg_update. sg_path_to_movie stays empty because the quicktime "
            "is generated in a temp dir, uploaded and deleted; a persistent "
            "movie requires 'batch_quicktime_template' in the pipeline "
            "config, which is empty today. Never invent a path for it.\n"
