...
CONCEPT_MAP: list[dict[str, str]] = [
    ...
    },
    {
        ...
        "notes": (
            ...
            "clip (steps Light plus compositing) -> create the review "
            "Version (export the movie, fill frames/movie paths, upload the "
            "streaming media). Execute STEP 8 of the fetched recipe EXACTLY; "
            "the only manual step left is update sources on the timeline."
        ),
    },
    {
        "concept": (
        ...
        "recipe": (
            ...
            "ACTIVE BATCH (falsified in-vivo, Chat 98): the current batch "
            "group CANNOT be switched from Python on Flame 2027 — "
            "bg.open() is a SILENT no-op when another batch is current, "
            "and open()+go_to() fails the same way. All wiring operates on "
            "flame.batch (the ACTIVE group): verify its name FIRST; if it "
            "is not the target, ask the operator to double-click the "
            "target group in the UI and wait — never retry open() loops. "
            "Guard every mutating idle event on the batch-group name so a "
            "wrong active batch is never touched.\n"
            "0) BATCH GROUP PER SHOT ('create a batch group per shot' / "
            "'setup batch'): ONE execute_plan call covers ALL shots — the "
            "op 'setup_comp_batch' (args: shot, clip_path, step, optional "
            "comp_dir) creates '<shot>_comp' with a 'sources' reel, imports "
            "the LIGHT RENDER (see below) and wires it to a Write File named "
            "'<shot>_<step>' (media-only, versioned; the batch start frame "
            "is derived from the source clip). STEP = the compositing "
            "Pipeline Step's short_name READ from ShotGrid (sg_find Step, "
            "field short_name — e.g. Comp -> 'CMP'), never guessed: it is "
            "what makes folder, frames and publish carry "
            "{Shot}_{Step}_v<version>, like the LGT publishes.\n"
            "   THE BATCH SOURCE IS THE LIGHT RENDER, NOT THE CONFORMED "
            "CLIP (Chat 99, in-vivo). The op derives the light publish's "
            "media pattern out of the clip and imports THAT. Importing the "
            "conformed clip is a FEEDBACK LOOP: it aggregates Light+Comp, so "
            "the moment a COMP version becomes current the source resolves "
            "to the comp's OWN OUTPUT — the comp composited over itself and "
            "rendered visually identical to the light pass, and rolling the "
            "comp media back left Flame spinning forever on 'Resize : Cannot "
            "access frame ... _CMP_v001.1035.exr', saturating the main "
            "thread. Nothing needs the aggregate as input since the Write "
            "File stopped creating clips: the clip AGGREGATES for the "
            "timeline flip, the batch READS the light render. When "
            "prepare_comp_render reports 'SOURCE LOOP', that batch predates "
            "this rule — re-point its source node at the light publish "
            "before rendering.\n"
            "   clip_path is still the CONFORMED clip (the op reads the "
            "light media path out of it): "
            "<root>/sequences/<Sequence>/<Shot>/finishing/clip/<Shot>.clip — "
            "the SAME file the conform timeline's segments point at. It "
            "EXISTS ON DISK (the conform creates it) and is NEVER published "
            "to ShotGrid: verify with a disk check, never conclude from SG "
            "publishes that it is missing, and NEVER generate a new .clip at "
            "a different path — a second clip file breaks the version-flip "
            "chain (in-vivo Chat 98: comp batches wired to a duplicate clip "
            "the timeline never looks at).\n"
            "   Send {'ops': [{'op': 'setup_comp_batch', 'args': {...}}, "
            "...]} with one entry per shot. Do not inspect selections or "
            "existing groups first; create, then verify with "
            "list_batch_groups. The op REPORTS which Write File attributes "
            "it set/skipped — relay that to the operator. FALLBACK ONLY (op "
            "unavailable): the template below, verbatim, fill SHOT and "
            "CLIP_PATH.\n"
            "```\n"
            "import flame, os, sys, io, json, time\n"
            "SHOT = 'SEQ001_SH001'\n"
            "CLIP_PATH = '/path/to/finishing/clip/' + SHOT + '.clip'\n"
            "_rp = '/tmp/flame_mcp_bb_%d_%d.json' % (os.getpid(), int(time.time() * 1000))\n"
            "def _do_build():\n"
            "    _buf = io.StringIO(); _old = sys.stdout; sys.stdout = _buf\n"
            "    try:\n"
            "        bg = flame.batch.create_batch_group(SHOT + '_comp', reels=['sources'])\n"
            "        node = flame.batch.import_clip(CLIP_PATH, 'sources')\n"
            "        print('OK: ' + SHOT + '_comp with source node')\n"
            "    except Exception as _e:\n"
            "        print('ERROR: ' + repr(_e))\n"
            "    finally:\n"
            "        sys.stdout = _old\n"
            "        try:\n"
            "            with open(_rp, 'w') as _fh: json.dump({'m': _buf.getvalue()}, _fh)\n"
            "        except Exception: pass\n"
            "flame.schedule_idle_event(_do_build)\n"
            "_dl = time.monotonic() + 15\n"
            "while time.monotonic() < _dl and not os.path.exists(_rp): time.sleep(0.25)\n"
            "print(open(_rp).read() if os.path.exists(_rp) else 'SCHEDULED: no result in 15s')\n"
            "```\n"
            "0b) For the wiring steps below, the shot's batch group already "
            "holds its source Clip node. Resolve it with flame.batch.get_node "
            "or by iterating batch.nodes — inside the idle event. Guard style "
            "that passes first time: give next() a default AND guard with "
            "'if x is None:' before use.\n"
            "1) EXPAND MULTILAYER: read the source Clip node's "
            "output_sockets (idle event) and report them. The multichannel "
            "EXR presents one output socket per layer. NOTE the VISUAL "
            "expansion of the node in the schematic (showing every layer "
            "connection) has NO Python API — it is the UI hotkey "
            "'Expand/Collapse Current Node'; tell the operator to press it "
            "if they want the node opened on screen, never attempt it "
            ...
            "2) SHADOW FIRST (operator-corrected wiring, Chat 98 in-vivo): "
            "create a Comp node (node_type from flame.batch.node_types), "
            "blend mode MULTIPLY. Wire beauty (rgba) -> FRONT, shadow layer "
            "-> BACK, charmatte -> the node's SECOND matte input (read the "
            "Comp node's input_sockets for the real socket names — never "
            "assume a single 'Matte'). INVERSION: dump the Comp node's FULL "
            "attributes list (a fuzzy 'invert' search is not a dump) and if "
            "a matte-invert option exists, set it ON THE NODE; only if it "
            "truly does not exist, route charmatte through a Negative node "
            "into that SECOND matte input.\n"
            "3) LIGHTS IN CASCADE: one Comp node per light/disco layer, "
            "blend mode SCREEN. THE RESULT TRAVELS ON FRONT: each Comp's "
            "Result -> the NEXT Comp's FRONT; the incoming light layer -> "
            "BACK. Order: lights first, club beams LAST (the review-mov "
            "formula: beauty x shadow, then lights, beams after).\n"
            "4) connect_nodes signature (verified on the 2027 graph): "
            "flame.batch.connect_nodes(output_node, output_socket_name, "
            "input_node, input_socket_name). Socket names come from reading "
            "the nodes — never guess them.\n"
            "5) CLOSE THE CHAIN: connect the LAST Comp's Result -> the "
            "shot's Write File FRONT (the batch already has it from step "
            "0). What stays manual is the 'Create Open Clip' check "
            "verification and the render itself — never the connection.\n"
            "6) LAYOUT: arrange the cascade graphically, top-left to "
            "bottom-right — source at top-left, each Comp stepped down-right, "
            "Write File at bottom-right — by setting each node's position "
            "attributes (pos_x/pos_y via node.attributes; dynamic surface, "
            "set defensively and report if absent). Do NOT rely on "
            ...
            "to the timeline') — the NATIVE path (Chat 99, validated "
            "in-vivo end to end). Flame's own tk-flame integration does the "
            "publishing and the review Version; our job is the Write File "
            "configuration, the open clip, and PROVING the segment anchor "
            "survived. Do NOT hand-roll publishes or quicktimes here — that "
            "was the previous design and it is gone.\n"
            "   a) run the execute_plan op prepare_comp_render (args: "
            "clip_path, step = the comp Step's short_name from ShotGrid, "
            "e.g. 'CMP'; idempotent). It configures the node so the output "
            "MATCHES the Toolkit templates, which is the gate the native "
            "hook checks — node named after the step, shot_name populated, "
            "media pattern '<name>_v<version>/<shot name>_<name>_v<version>."
            "<frame>', setup redirected to '../batch/<shot name>.v<version>', "
            "create_clip ON but pointing at the node's OWN clip in the comp "
            "folder, frame padding and start frame derived from the source. "
            "It then SAVES the batch: attribute changes live in memory only "
            "and a Flame restart silently reverts them (Chat 99). Relay the "
            "'ALIGNMENT:' line VERBATIM — both frame counts included, never a "
            "paraphrase — and render only on '-> OK'. The line carries the "
            "write file range AND the source range precisely so the mismatch "
            "is visible: Chat 101 summarised it away as 'aligned, 100 frames' "
            "while the node was set to 1001-1002, and the shot rendered 2 "
            "frames of 100 into a delivery nobody caught until the movie was "
            "two frames long.\n"
            "   b) render with the render_batch tool, render_option "
            "**Foreground** — NOT Background Reactor, and this overrides the "
            "engine's usual 'always use Background Reactor' rule. Flame "
            "fires batchExportEnd when the job is SENT, not when it "
            "finishes, so a background render makes tk-flame's publish chain "
            "run against frames that do not exist yet: measured in-vivo, the "
            "job was sent at 16:27:21, the hook warned 'does not exist' at "
            "16:27:25, the Reactor finished frame 1100 at 16:28:08 and the "
            "quicktime transcode died at 16:28:13 with 'cannot be imported "
            "to be transcoded'. The publishes and the Version still land — "
            "only the movie is lost — which makes the failure easy to miss. "
            "Flame pops its own 'Send to Review' modal: the OPERATOR accepts it with a comment "
            "— that is what arms the review, and it is deliberately part of "
            "the on-camera flow. Do not queue more work on Flame's "
            "main thread while a modal is open.\n"
            "      HOW TO WAIT — you own this, the operator does not. In "
            "Foreground flame.batch.render() BLOCKS, so the tool's outcome "
            "file is written when the render ENDS, not when it starts; the "
            "tool deletes it before scheduling so its reappearance is the "
            "signal. Poll DISK ONLY, never Flame, in a loop with a sleep of a "
            "few seconds: count the .exr files in the version folder the "
            "'render target:' line named, and stop when the count equals "
            "range_end - range_start + 1 from the ALIGNMENT line, no file is "
            "zero bytes, the count is stable across two consecutive reads, "
            "and ~/flame_render_result.txt reads 'OK: render finished'. "
            "'ERROR: ...' there means the render aborted — read_flame_log for "
            "the cause; do not retry blind. Budget roughly 0.2 s per frame "
            "(100 frames rendered in ~17 s in-vivo) and give up after a "
            "generous timeout with a plain report of the counts you saw.\n"
            "      NEVER hand the wheel back with 'tell me when it has "
            "finished'. Chat 100 and Chat 101 both ended that way, and both "
            "times steps (e) and (f) below never ran, so the conformed clip "
            "was left with no comp layer and the shelf clip unrenamed. The "
            "ONLY thing the operator owns in this step is accepting the "
            "modal; detecting the render's end is yours.\n"
            "   c) tk-flame then publishes the .batch ('Flame Batch File') "
            "and the rendered sequence ('Flame Render'), creates the Version "
            "with sg_path_to_frames and the frame range, and uploads the "
            "quicktime. Nothing for us to do — verify it landed.\n"
            "   d) LINK THE TASK — the only thing the native path leaves "
            "undone. The Version and both publishes come back with NO Task "
            "because the context is resolved from the .batch path and "
            "flame_shot_batch carries no {Step} token, so Toolkit resolves a "
            "Shot and stops there. Close it in the same breath as the "
            "delivery: sg_find the Shot's Task whose Step short_name is the "
            "one you configured (e.g. CMP), then sg_update the Version "
            "(sg_task) and both PublishedFiles (task). Do not skip it and do "
            "not report DONE without it — an unlinked publish is invisible "
            "to every Task-based query in production.\n"
            "      The review movie needs nothing from us: with "
            "'batch_quicktime_template' configured (flame_shot_comp_mov, "
            "Chat 99) tk-flame writes a permanent quicktime next to the "
            "light one, publishes it as 'Flame Quicktime' and its backburner "
            "hook fills sg_path_to_movie itself. If that field comes back "
            "empty, the template is missing from the config — say so; never "
            "invent a path to a movie that was generated in a temp dir and "
            "deleted.\n"
            "   e) regenerate the conformed clip: openclip_create "
            "steps=['LGT'], extra_publish_types=['Flame Render'], "
            "keep_source_current=True. Pass the Step SHORT_NAME ('LGT'), not "
            "its code ('Light'): the selector matches either, but the string "
            "you pass becomes the uid prefix uppercased, so 'Light' silently "
            "relabels the light layer 'LIGHT_v003' where the conformed clip "
            "already carries 'LGT_v003'. And it must be the LIST form — the "
            "singular step= leaves the layer with a bare 'v003' uid. "
            "extra_publish_types is REQUIRED because of the publish TYPE, not "
            "a missing Task (Chat 100 measured the opposite of what Chat 99 "
            "assumed: the native publishes DO carry a Task once step (d) "
            "links them). The per-Step selector filters "
            "publish_type='Rendered Image' and the native comp publish is "
            "'Flame Render', so with or without a Task the Step rounds always "
            "skip it. keep_source_current is "
            "REQUIRED because Flame reads a clip's start frame from its "
            "CURRENT version — measured, the same file reads start_frame "
            "1001 with the light version current and 0 with the comp "
            "current, and at 0 every 'Update Sources' replace anchors the "
            "conformed segment at 00:00:00:00 and loses its cut.\n"
            "   f) RENAME THE RENDERED CLIP, after the open clip is "
            "regenerated (operator request, Chat 99). The Write File's "
            "'Add to Workspace' drops the render into the batch's 'Batch "
            "Renders' shelf reel named after the NODE — which the Toolkit "
            "template forces to be the bare step ('CMP'), because "
            "{segment_name} is what builds the media folder. On screen that "
            "reads as an anonymous 'CMP'. Rename it to "
            "<Shot>_<step>_v<version>: find the clip in bg.shelf_reels and "
            "assign clip.name (a plain attribute assignment; do NOT touch "
            "the node name, which the template depends on).\n"
            "   g) VERIFY THE ANCHOR of every segment of the conformed "
            "sequence and report it in the same breath as the delivery: "
            "each source_in must equal the source's first frame as timecode "
            "(frame 1001 at 25 fps = 00:00:40:01). This check is not "
            "optional — the failure is silent, and Update Sources CANNOT "
            "repair it afterwards (verified across all three "
            "version_selection_mode values). A damaged segment must be "
            "re-laid: import the clip and PySequence.overwrite(clip, "
            "PyTime(record_in.frame + 1)) — record_frame is one-based while "
            "record_in reads back zero-based.\n"
            "   h) the version FLIP stays MANUAL. Never switch versions for "
            "the operator: showing that both are available and choosing one "
            "on camera is the point of the demo. update_sources() with a "
            "selection mode exists and works, but it is for repair "
            "diagnostics, not for the delivery cycle.\n"
            "   i) report DONE with: the publish ids, the Version id, the "
            "anchor verdict, and the one manual step left — flip the version "
            "on the timeline."
        ),
    },
    {
        ...
        "recipe": (
            ...
            "2) One open clip per shot with openclip_create. Call it with "
            "steps=[<the chosen step>] — the LIST form — never the singular "
            "step=. Both build the same clip today, but the list form "
            "prefixes the version uid with the step ('LIGHT_v003' instead of "
            "'v003'), and that is what the comp delivery cycle will "
            "regenerate later. Using the singular form here means the uid "
            "CHANGES under an already-conformed segment the first time a "
            "comp is delivered (Chat 99).\n"
