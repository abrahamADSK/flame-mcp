...
CONCEPT_MAP: list[dict[str, str]] = [
    ...
    },
    {
        ...
        "recipe": (
            ...
            "repeat) and relay its set/skipped report. FRAME ALIGNMENT: "
            "pass start_frame = the source media's FIRST frame (read it "
            "from the source publish filenames, e.g. 1001 — never assume): "
            "a comp rendered from frame 1 misaligns against the source in "
            "the shared clip and the LIGHT version shows 'no media' on "
            "flip (in-vivo). setup_comp_batch takes the same start_frame "
            "at creation;\n"
