...
CONCEPT_MAP: list[dict[str, str]] = [
    ...
    },
    {
        ...
        "recipe": (
            ...
            "conformed clip, so the timeline flips natively). Send "
            "{'ops': [{'op': 'setup_comp_batch', 'args': {...}}, ...]} with "
            "one entry per shot. Do not inspect selections or existing "
            "groups first; create, then verify with list_batch_groups. The "
            "op REPORTS which Write File attributes it set/skipped — relay "
            "that to the operator. FALLBACK ONLY (op unavailable): the "
            "template below, verbatim, fill SHOT and CLIP_PATH.\n"
            "```\n"
