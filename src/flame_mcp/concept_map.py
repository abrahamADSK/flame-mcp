...
CONCEPT_MAP: list[dict[str, str]] = [
    ...
    },
    {
        ...
        "recipe": (
            ...
            "0b) ONE OP COVERS STEPS 1-6 (Chat 100): the execute_plan op "
            "'wire_comp_tree' (args: optional batch_group, optional "
            "dry_run) reads the source sockets, classifies the layers, wires "
            "beauty x shadow (MULTIPLY, charmatte on the SECOND matte input, "
            "inverted when the node exposes it), cascades one SCREEN Comp per "
            "light with the club beams LAST, closes into the Write File and "
            "lays the graph out diagonally — all inside one idle event. "
            "batch_group EMPTY = the active group; NAMING a group wires THAT "
            "group WITHOUT activating it, which was measured in-vivo (Chat "
            "100: the node lands in the target, the active group is "
            "untouched, deletion is clean). So several shots compose in ONE "
            "plan — send one op per shot with its batch_group — and the "
            "operator never double-clicks. The active batch still cannot be "
            "SWITCHED from Python; this sidesteps that, it does not fix it. "
            "Run it with dry_run first when the layer names are unknown: it "
            "reports the classification, including what it IGNORED, and "
            "wires nothing. The manual sequence below stays as the fallback "
            "and as the specification the op implements.\n"
            ...
            "and a Flame restart silently reverts them (Chat 99). Relay its "
            "report, and render only on 'ALIGNMENT: ... -> OK'.\n"
            ...
            "the on-camera flow. WAIT; do not queue more work on Flame's "
            ...
            "steps=['Light'], extra_publish_types=['Flame Render'], "
            "keep_source_current=True. The type selector is REQUIRED because "
            "the native publish has no Task; keep_source_current is "
